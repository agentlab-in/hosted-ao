...
// ~/.ao/hosted layout or the real daemon/storage. The on-disk boundary is
// exercised only in the packaged-app pod gate (#2697), which today runs a
// boot-level smoke (app launches, daemon ready), NOT the full ~/.ao/hosted
// layout assertion — that stays in the pod data-dir script as future
// per-case work.
//
// The on-disk checks (Electron userData resolves under ~/.ao/hosted/electron,
// daemon data under ~/.ao/hosted, no OS-default app-data writes, AO_DATA_DIR
// override) run in the pod against a packaged build — they are not
// observable from the renderer. What the renderer CAN attest is the
// daemon-readiness contract that those invariants gate: a daemon only
// advertises its REST port once its data dir + config skeleton are
// initialized, and the renderer reflects that as a ready status and a
// hydrated board. This locks the renderer side of the invariant; the
// ~/.ao/hosted filesystem assertions stay in the pod data-dir script.
