...
// #2483 INS-001.
test("renderer: packaged bundle launches and paints @T0 @INS", async ({
  page,
}) => {
...
// #2483 INS-007.
test("renderer: update settings surface renders (feed/checksum checks are pod) @T0 @INS", async ({
  page,
}) => {
  ...
  await page.getByRole("button", { name: "Updates" }).click();
  await expect(
    page.locator('[data-testid="settings-section"][data-section="updates"]'),
  ).toBeVisible();
  await expect(page.getByTestId("app-version")).toContainText("v9.9.9-test");
});

// #2483 INS-002.
test("renderer: first-run home renders with the app launched @T0 @INS", async ({
  page,
}) => {
...
// #2483 INS-003.
test("renderer: reflects a ready daemon (data dir + config initialized) @T0 @INS", async ({
  page,
}) => {
...
// #2483 INS-004.
test("renderer: app version string renders as expected @T0 @INS", async ({
  page,
}) => {
  await installFakeBridge(page, { version: "9.9.9-test" });
  await page.goto("/#/settings");
...
// #2483 DMN-001.
test("renderer: reflects the daemon reaching ready on app start @T0 @DMN", async ({
  page,
}) => {
  await installFakeBridge(page, { daemonState: "ready", daemonPort: 8080 });
  await page.goto("/");
...
// #2483 DMN-002.
test("renderer: daemon health reflected with a hydrated board @T0 @DMN", async ({
  page,
}) => {
...
// #2483 DMN-005.
test("renderer: daemon stop surfaced cleanly with no renderer crash @T0 @DMN", async ({
  page,
}) => {
...
// #2483 DMN-009.
test("renderer: board state rehydrates after a renderer relaunch @T0 @DMN", async ({
  page,
}) => {
  ...
  await expect(page.getByTestId("board-session-card").first()).toBeVisible();
  expect(
    await page.getByTestId("board-session-card").first().textContent(),
  ).toBe(before);
});
...
test("renderer: board renders all status columns @T0 @BRD", async ({
  ...
  await expect(
    page.locator('[data-testid="board-column"][data-column="working"]'),
  ).toContainText("Working");
  await expect(
    page.locator('[data-testid="board-column"][data-column="action"]'),
  ).toContainText("Needs you");
  await expect(
    page.locator('[data-testid="board-column"][data-column="pending"]'),
  ).toContainText("In review");
  await expect(
    page.locator('[data-testid="board-column"][data-column="merge"]'),
