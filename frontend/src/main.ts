...
let daemonStatus: DaemonStatus = remoteDaemonLifecycle.currentStatus() ?? {
  state: "stopped",
};
let daemonOutput = "";
...
function buildWindowsAppMenu(): Menu {
  return Menu.buildFromTemplate(
    buildWindowsAppMenuTemplate(toggleFocusedDevTools),
  );
}
...
function browserProfileStateDir(): string {
  ...
  return path.dirname(
    runFile ?? path.join(os.homedir(), ...STATE_ROOT_SEGMENTS, "running.json"),
  );
}
...
async function createWindowInternal(): Promise<void> {
  ...
  const agentBrowserRuntime = new AgentBrowserRuntime({
    ...
    // 103-byte sockaddr_un limit; all AO state remains under the hosted state
    // root, which STATE_ROOT_SEGMENTS spells so this cannot drift from the
    // daemon's ~/.ao/hosted (a bare ~/.ao would share a run root, and its
    // sockets, with the upstream agent-orchestrator build).
    dataDir: path.join(
      os.homedir(),
      ...STATE_ROOT_SEGMENTS,
      ...(app.isPackaged ? ["br"] : [DEV_STATE_SUBDIR, "br"]),
    ),
...
const SUPERVISOR_PIPE_SENTINEL =
  STATE_ROOT_SEGMENTS[STATE_ROOT_SEGMENTS.length - 1];
const SUPERVISOR_PIPE_BASE_NAME =
  "\\\\.\\pipe\\ao-supervise-" + SUPERVISOR_PIPE_SENTINEL;
...
function supervisorPipeFromRunFile(rfp: string | null): string {
  ...
  if (dir === SUPERVISOR_PIPE_SENTINEL || dir === "." || dir === "")
    return SUPERVISOR_PIPE_BASE_NAME;
...
async function refreshDaemonStatus(): Promise<DaemonStatus> {
  return remoteDaemonLifecycle.refresh(
...
ipcMain.handle("editorHandoff:open", (event, input) => {
  ...
  return editorHandoff.open(
    input && typeof input === "object" ? input : { sessionId: "" },
  );
});
  ...
  if (scheme === "light" || scheme === "dark") {
    try {
      await persistSelectedDaemonTerminalTheme(scheme, {
        isRemoteSelected: () =>
          machineSelection().getState().activeMachineId !== LOCAL_MACHINE_ID,
...
let peerWorkspacesController: ReturnType<
  typeof createPeerWorkspacesController
> | null = null;
...
ipcMain.handle("aoMachines:select", (_event, machineId: string) =>
  machineSelection().select(machineId),
);
...
ipcMain.on(
  "pairedMachines:cancelFingerprintProbe",
...
ipcMain.handle("pairedMachines:remove", (_event, id: string) =>
  pairedMachines().remove(id),
);
...
app.whenReady().then(async () => {
  ...
  setInterval(() => {
    if (policyController.snapshot().state !== "applied")
  ...
  session.defaultSession.setCertificateVerifyProc(
    pairedMachines().verifyCertificate,
  );
