...
export const recentNotificationsQueryKey = [
  "notifications",
  "history",
  "all",
] as const;
...
export async function fetchNotificationsPage(
  ...
  if (error)
    throw new Error(apiErrorMessage(error, "Could not load notifications"));
  ...
  return {
    ...
    unresolvedCount:
      data?.unresolvedCount ?? notifications.filter(isUnresolved).length,
  };
}
...
export async function markAllNotificationsRead(ids: string[]): Promise<number> {
  ...
  if (error)
    throw new Error(
      apiErrorMessage(error, "Could not mark notifications read"),
    );
  return data?.updatedCount ?? 0;
}

export function mergeUnreadNotification(
  queryClient: QueryClient,
  notification: NotificationDTO,
): boolean {
  if (notification.status !== "unread") return false;
  const inserted = mergeNotificationIntoCache(
    queryClient,
    unreadNotificationsQueryKey,
    notification,
  );
...
function mergeRecentNotification(
  queryClient: QueryClient,
  notification: NotificationDTO,
): boolean {
  const inserted = mergeNotificationIntoCache(
 ...
 */
export function applyResolvedNotification(
  queryClient: QueryClient,
  notification: NotificationDTO,
): void {
...
function mergeNotificationIntoCache(
  ...
  queryClient.setQueryData<NotificationsCache>(queryKey, (current) => {
    ...
    const existing = getCachedNotifications(current).find(
      (item) => item.id === notification.id,
    );
    const unreadDelta =
      (notification.status === "unread" ? 1 : 0) -
      (existing?.status === "unread" ? 1 : 0);
    const unresolvedDelta =
      (isUnresolved(notification) ? 1 : 0) -
      (existing && isUnresolved(existing) ? 1 : 0);
...
export function getCachedUnreadCount(
  ...
  return (
    cache?.pages[0]?.unreadCount ??
    getCachedNotifications(cache).filter((item) => item.status === "unread")
      .length
  );
}
...
function suppressToastForWatchedSession(
  ...
  if (
    ...
    notification.sessionId !== visibleAgentSessionId
  )
    return false;
      ...
      const connectSource = () => {
        ...
        try {
          source = new EventSource(
            `${baseUrl.replace(/\/+$/, "")}/api/v1/notifications/stream`,
            {
          ...
          source.addEventListener("notification_created", (event) => {
            ...
            if (
              inserted &&
              !suppressToastForWatchedSession(
                notification,
                getVisibleAgentSessionId(),
              )
            ) {
...
function rebaseOversizedFirstPage(
  queryClient: QueryClient,
  queryKey: NotificationsQueryKey,
): void {
  const cache = queryClient.getQueryData<NotificationsCache>(queryKey);
  if (!cache || cache.pages[0]?.notifications.length <= NOTIFICATION_PAGE_SIZE)
    return;
    ...
    queryClient.setQueryData<NotificationsCache>(queryKey, (current) => {
      ...
      return {
        ...
        pages: [
          {
            ...current.pages[0],
            notifications: current.pages[0].notifications.slice(
              0,
