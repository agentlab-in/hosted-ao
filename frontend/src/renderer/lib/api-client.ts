...
const initialApiBaseUrl =
  explicitApiBaseUrl ??
  (import.meta.env.DEV ? devApiBaseUrl() : "http://127.0.0.1:3001");
...
export function setApiBaseUrl(nextBaseUrl: string | null): void {
  const normalized =
    (nextBaseUrl ?? explicitApiBaseUrl ?? null)?.replace(/\/+$/, "") ?? null;
...
function matchRouteTemplate(
  pathname: string,
  template: string,
): { normalized: string; score: number } | null {
...
export function normalizeApiOperation(
  method: string,
  pathname: string,
): string {
...
type ApiErrorCategory =
  ...
  | "http_4xx"
  | "http_5xx";
type ReportingOwner = NonNullable<
  components["schemas"]["APIError"]["reporting_owner"]
>;
...
function reportApiError(
  ...
  const ownerBucket =
    reportingOwner === "agent_switch_saga" ? "agent_switch_saga" : "http";
...
async function runtimeFetch(input: Request): Promise<Response> {
  const operation = normalizeApiOperation(
    input.method,
    new URL(input.url).pathname,
  );
  const visibilityOwned =
    operation === "GET /api/v1/projects" ||
    operation === "GET /api/v1/sessions" ||
    operation === "GET /api/v1/sessions/:id/agent-switches";
  ...
  if (remote && !gatewayToken) {
    ...
    if (!visibilityOwned) reportApiError(operation, "machine_token_unavailable", 503);
    return new Response(
      JSON.stringify({
        message:
          "This machine's sign-in is not available right now. Reconnecting.",
  ...
  const body =
    passthrough || input.method === "GET" || input.method === "HEAD"
      ? undefined
  ...
  const sendReportingFailure = async (
    token: string | null,
  ): Promise<Response> => {
    try {
  ...
  if (!response.ok) {
    ...
    try {
      ...
      if (
        body?.reporting_owner === "http" ||
        body?.reporting_owner === "agent_switch_saga"
      ) {
...
export function apiErrorMessage(
  ...
  if (typeof error === "object" && error !== null) {
    ...
    if (typeof body.message === "string" && body.message !== "") {
      return code && !body.message.includes(code)
        ? `${body.message} (${code})`
        : body.message;
    }
