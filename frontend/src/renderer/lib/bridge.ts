...
const previewRemoteMachine = (
  machine: Partial<AoMachine> & Pick<AoMachine, "id" | "name" | "baseUrl">,
): AoMachine => ({
  local: false,
  createdAt: null,
  lastSeen: null,
...
const browserPreviewPeerWorkspaces = (): PeerWorkspacesResult => {
  if (previewActiveMachineId === LOCAL_MACHINE_ID) {
    const peer = BROWSER_PREVIEW_MACHINES.find(
      (machine) => machine.id === "mch_ready",
    );
    if (!peer)
      return { state: "unavailable", reason: "No cloud machine registered." };
    return {
      state: "ok",
      machineId: peer.id,
...
const browserPreviewDaemonStatus = (): DaemonStatus => {
  const active = BROWSER_PREVIEW_MACHINES.find(
    (machine) => machine.id === previewActiveMachineId,
  );
