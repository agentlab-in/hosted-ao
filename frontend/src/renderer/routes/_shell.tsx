...
function findRegisteredWorkspaceByPath(
  workspaces: WorkspaceSummary[],
  path: string,
): WorkspaceSummary | undefined {
  const normalizedPath = normalizeProjectPath(path);
  return workspaces.find(
    (workspace) => normalizeProjectPath(workspace.path) === normalizedPath,
  );
}
...
): components["schemas"]["ProjectConfig"] {
  return {
    ...(input.defaultBranch ? { defaultBranch: input.defaultBranch } : {}),
    worker: {
      agent:
        input.workerAgent as components["schemas"]["RoleOverride"]["agent"],
    },
    orchestrator: {
      agent:
        input.orchestratorAgent as components["schemas"]["RoleOverride"]["agent"],
    },
...
const ShellCenter = memo(function ShellCenter({
  ...
  const panelClassName = isSessionRoute
    ? "center-panel-shell--session"
    : undefined;
  if (hideShellTopbar) {
...
function ShellLayout() {
  ...
  const [workspaceStartupState, setWorkspaceStartupState] = useState<
    "loading" | "ready" | "error"
  >("loading");
  ...
  const requestCreateProjectFromPath = useUiStore(
    (state) => state.requestCreateProjectFromPath,
  );
  const requestNewShellTerminal = useUiStore(
    (state) => state.requestNewShellTerminal,
  );
  const newShellTerminalNonce = useUiStore(
    (state) => state.newShellTerminalNonce,
  );
  ...
  const [isKeyboardShortcutsSettingsOpen, setIsKeyboardShortcutsSettingsOpen] =
    useState(false);
  const routeParams = useParams({ strict: false }) as {
    projectId?: string;
    sessionId?: string;
  };
  const linkSession = workspaces
    .flatMap((workspace) => workspace.sessions)
    .find((session) => session.id === routeParams.sessionId);
  const openBrowserLink = useSessionBrowserLink(linkSession);
  const canOpenBrowserLink =
    linkSession?.kind === "worker" && sessionIsActive(linkSession);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const handleDragEnter = (event: DragEvent) => {
      ...
      if (dragDepthRef.current === 1 && firstEntryIsDirectory(event))
    ...
    void queryClient
      .prefetchQuery({
        ...
        queryFn: async () => {
          ...
          if (data?.status !== "ok")
            throw new Error("Project config unavailable");
          return data.project as components["schemas"]["Project"];
        },
      })
      .then(() => {
        const project =
          queryClient.getQueryData<components["schemas"]["Project"]>(
            projectQueryKey,
          );
        const defaultWorkerAgent =
          project?.config?.worker?.agent || project?.agent || "";
        if (defaultWorkerAgent) {
          void queryClient.prefetchQuery(
            agentModelsQueryOptions(defaultWorkerAgent, scopedProjectId),
          );
        }
      });
  ...
  const hideShellTopbar =
    isHomeRoute || selfFramedCenterPanel || shellTopbarHiddenByPlatform;
  const setProjectRestarting = useUiStore(
    (state) => state.setProjectRestarting,
  );
  const orchestratorReplacementErrors = useUiStore(
    (state) => state.orchestratorReplacementErrors,
  );
  ...
  const setProjectProvisioning = useUiStore(
    (state) => state.setProjectProvisioning,
  );
  const showGlobalToast = useUiStore((state) => state.showGlobalToast);
  const replacementErrorProjectId =
    Object.keys(orchestratorReplacementErrors)[0] ?? null;
    ...
    (direction: -1 | 1) => {
      ...
      const sessions = (
        workspacesRef.current.find(
          (workspace) => workspace.id === scopedProjectId,
        )?.sessions ?? []
    ...
    async (input: {
      ...
      const { data, error } = await apiClient.POST("/api/v1/projects", {
        body: {
          ...
          // PATH_AND_CLONE_URL_CONFLICT, and the input makes the pair
          // unrepresentable upstream of here.
          ...(input.cloneUrl
  ...
  const removeProject = useCallback(
    async (projectId: string) => {
      ...
      void captureRendererEvent("ao.renderer.project_removed", {
        project_id: projectId,
      });
  ...
  useEffect(() => {
    ...
    void queryClient
      .fetchQuery({ ...workspaceQueryOptions, staleTime: 0 })
          .then((workspaces) => {
            if (active && !workspaceStatusesChecking(workspaces))
              setWorkspaceStartupState("ready");
          })
      .catch((error) => {
  ...
  useEffect(() => {
    const handleKeyDown = (event: KeyboardEvent) => {
      ...
      if (matchesRendererShortcut("open-project", event)) {
        ...
        const workspace =
          workspacesWithoutStandalone[Number(event.key) - 1];
        if (workspace) {
          event.preventDefault();
          void navigate({
  ...
  useEffect(
    () =>
      aoBridge.app.onKeyboardShortcutsHelp(() =>
  ...
  useEffect(
    () =>
      aoBridge.app.onOpenSettingsShortcut(() =>
  ...
  useEffect(() => {
    const disposePrevious = aoBridge.app.onPreviousSessionShortcut(() =>
      navigateSession(-1),
    );
    const disposeNext = aoBridge.app.onNextSessionShortcut(() =>
      navigateSession(1),
    );
    return () => {
      disposePrevious();
  ...
  return (
    <ShellProvider value={shellContextValue}>
      <AppBrowserLinkContext.Provider
        value={canOpenBrowserLink ? openBrowserLink : undefined}
      >
        ...
        >
          ...
          >
            ...
            {!framedAppTopbar && !hideShellTopbar && !routeParams.sessionId ? (
              <ShellTopbar />
            ) : null}
              ...
              below its custom titlebar. */}
                ...
                <main
                  className={cn(
                    ...
                    !sidebarHasLayout && "sidebar-hidden",
                  )}
                >
            ...
            <OrchestratorReplacementDialog
              pending={Boolean(
                replacementErrorProjectId &&
                  restartingProjectIds.has(replacementErrorProjectId),
              )}
              error={
                replacementErrorProjectId
                  ? orchestratorReplacementErrors[replacementErrorProjectId]
                  : undefined
              }
              onOpenChange={(open) => {
              ...
              onRetryAsTui={(projectId) =>
                void restartOrchestrator(projectId, "tui")
              }
