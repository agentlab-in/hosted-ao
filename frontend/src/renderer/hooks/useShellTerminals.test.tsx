...
describe("useOpenShellTerminal", () => {
  ...
  it("sends the saved Windows shell preference to the daemon", async () => {
    ...
    await act(async () =>
      result.current.mutateAsync({ projectId: "project-1" }),
    );
  ...
  it("lets an explicit shell override the saved preference", async () => {
    ...
    await act(async () =>
      result.current.mutateAsync({ shell: "C:\\Tools\\bash.exe" }),
    );
  ...
  it("normalizes an unavailable configured shell back to Automatic", async () => {
    isWindowsMock.mockReturnValue(true);
    shellStoreMock.preference = {
      kind: "custom",
      path: "C:\\missing\\shell.exe",
    };
    postMock.mockResolvedValue({
      error: { code: "SHELL_TERMINAL_SHELL_UNAVAILABLE" },
    });
    const queryClient = new QueryClient({
      defaultOptions: { mutations: { retry: false } },
    });
    ...
    await waitFor(() =>
      expect(shellStoreMock.setPreference).toHaveBeenCalledWith({
        kind: "auto",
      }),
    );
  });
...
describe("useRenameShellTerminal", () => {
  it("updates the visible tab title before the daemon responds", async () => {
    let finishRename!: (result: {
      data: { shellTerminal: ShellTerminal };
    }) => void;
    patchMock.mockReturnValue(
      new Promise((resolve) => (finishRename = resolve)),
    );
    const queryClient = queryClientWithShells();
    const { result } = renderHook(() => useRenameShellTerminal(), {
      wrapper: wrapper(queryClient),
    });

    act(() =>
      result.current.mutate({
        handleId: shells[0].handleId,
        title: "server, zsh",
      }),
    );
    ...
    await waitFor(() =>
      expect(
        queryClient.getQueryData<ShellTerminal[]>(shellTerminalsQueryKey)?.[0]
          ?.title,
      ).toBe("server, zsh"),
    );
    act(() =>
      finishRename({
        data: { shellTerminal: { ...shells[0], title: "server, zsh" } },
      }),
    );
...
describe("useCloseShellTerminal", () => {
  it("removes the terminal tab before an in-flight list request finishes cancelling", async () => {
    ...
    const { result } = renderHook(() => useCloseShellTerminal(), {
      wrapper: wrapper(queryClient),
    });
  ...
  it("does not restore a stale tab when the daemon reports that its PTY is already gone", async () => {
    deleteMock.mockResolvedValue({
      error: { code: "SHELL_TERMINAL_NOT_FOUND" },
    });
    const queryClient = queryClientWithShells();
    const { result } = renderHook(() => useCloseShellTerminal(), {
      wrapper: wrapper(queryClient),
    ...
    await expect(
      result.current.mutateAsync(shells[0].handleId),
