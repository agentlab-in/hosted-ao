...
export type CreateProjectSource =
  | { path: string; cloneUrl?: never }
  | { cloneUrl: string; path?: never };

export type CreateProjectInput = CreateProjectSource & {
  asWorkspace?: boolean;
...
export function CreateProjectFlow({
  ...
  const [projectImportStep, setProjectImportStep] =
    useState<ProjectImportStep | null>(null);
  const [projectPrepEvents, setProjectPrepEvents] = useState<
    GitPreparationEvent[]
  >([]);
  const [projectApprovedActions, setProjectApprovedActions] = useState<
    string[]
  >([]);
  ...
  const projectImportOpen =
    projectImportStep !== null && projectValidation !== null;
  const isBusy =
    isChoosingPath || isCreating || isInitializing || isPreparingGit;
      ...
      if (path && kind === "single_repo") {
        ...
        setProjectRemoteUrl(
          validation.root.requiredActions.includes("set_remote")
            ? suggestedProjectRemoteUrl(validation.root.repoPath)
            : "",
        );
        setProjectSuggestWorkspace(
      ...
      if (cloneSelection) {
        ...
        if (cloneSelection.destinationParent === "") {
          await onCreateProject({
            cloneUrl: cloneSelection.remoteUrl,
            ...selection,
          });
        } else {
  ...
  const prepareProjectGit = async () => {
    ...
    setError(null);
    setProjectPrepEvents(
      projectRequestedActionEvents(
        projectValidation.root.repoPath,
        projectApprovedActions,
      ),
    );
    ...
    try {
      ...
      if (apiError || !data)
        throw new Error(
          apiErrorMessage(apiError, t("createProject.couldNotAdd")),
        );
      ...
      if (projectRemoteUrl.trim() !== "")
        persistSuggestedProjectRemoteUrl(projectRemoteUrl);
      ...
      {hasModePicker && (
        <>
          <CreateProjectSourceDialog
            childOpen={
              childTransitioning || cloneDialogOpen || folderPickerOpen
            }
...
async function validateImportFolder(
  ...
  if (error || !data)
    throw new Error(apiErrorMessage(error, "Could not validate this folder."));
  return data;
}

function importValidationMessage(result: ImportValidationResult): string {
  if (result.blockingErrors.length === 0)
    return "This folder cannot be imported yet.";
...
function latestProjectActionState(
  action: string,
...
function projectRequestedActionEvents(
  repoPath: string,
  actions: string[],
): GitPreparationEvent[] {
...
function suggestedProjectRemoteUrl(repoPath: string): string {
  ...
  const saved =
    window.localStorage.getItem(LAST_IMPORT_REMOTE_URL_KEY)?.trim() ?? "";
  if (!repoName) return saved;
  const withGitSuffix = repoName.endsWith(".git")
    ? repoName
    : `${repoName}.git`;
...
function ImportSourcePicker({
  ...
  const sources: Array<{
    ...
    },
    {
      source: "local",
      icon: (
        <FolderClosed className="size-5" aria-hidden="true" strokeWidth={1.8} />
      ),
  ...
  return (
    <div className="relative w-full max-w-[520px] overflow-hidden rounded-lg border border-border bg-popover text-popover-foreground shadow-xl">
      {dialog ? (
        <Dialog.Title className="settings-dialog-title px-4 pt-3">
          {t("createProject.addCodeTitle")}
        </Dialog.Title>
      ) : (
        <h2 className="settings-dialog-title px-4 pt-3">
          {t("createProject.addCodeTitle")}
        </h2>
      )}
      {dialog ? (
...
function ProjectImportDialog({
  ...
  const missingApprovals = validation.root.requiredActions.filter(
    (action) => !approvedActions.includes(action),
  );
  const continueDisabled =
    disabled ||
    missingApprovals.length > 0 ||
    (needsRemote && remoteUrl.trim() === "");
  return (
    <Dialog.Root open={open} onOpenChange={onOpenChange}>
      <Dialog.Portal>
        <Dialog.Overlay className="fixed inset-0 z-overlay bg-black/55 data-[state=open]:animate-overlay-in data-[state=closed]:animate-overlay-out motion-reduce:animate-none" />
        ...
        >
          ...
          <div className="min-h-0 space-y-4 overflow-y-auto px-4 py-4">
            <div className="flex items-center gap-3 rounded-md border border-[var(--color-border-import-modal)] bg-[var(--color-bg-import-card)] px-3 py-2.5">
              <Folder
                className="size-4 shrink-0 text-[var(--color-text-import-muted)]"
                aria-hidden="true"
              />
              <div className="min-w-0 flex-1">
            ...
            {error ? (
              <div
                className="rounded-lg border border-destructive/40 bg-destructive/10 px-4 py-3 text-[12px] leading-5 text-destructive"
                role="alert"
              >
                {error}
              </div>
            ...
            {step === "prepare_git" ? (
              <section className="space-y-3">
                <div className="rounded-lg border border-[var(--color-border-import-modal)] bg-[var(--color-bg-import-card)] px-4 py-4">
                  <h3 className="text-[13px] font-semibold text-[var(--color-text-import-title)]">
                    {t("createProject.projectSetup")}
                  </h3>
                            ...
                            {action === "set_remote" ? (
                              <span className="mt-3 block space-y-2">
                                ...
                                <Input
                                  ...
                                  disabled={disabled}
                                  placeholder={t(
                                    "createProject.cloneRepositoryUrlPlaceholder",
                                  )}
                                  spellCheck={false}
                                  value={remoteUrl}
                                  onChange={(event) =>
                                    onChangeRemote(event.target.value)
                                  }
                                />
              ...
              <div className="space-y-3">
                ...
                {error && (
                  <div className="rounded-lg border border-destructive/40 bg-destructive/10">
                    ...
                    <div className="px-3 py-2 text-[12px] leading-5 text-destructive">
                      {error}
                    </div>
