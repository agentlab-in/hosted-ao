...
export function SettingsDialog() {
  ...
  const globalSections: Array<{
    ...
    {
      id: "browserProfiles",
      label: t("settings.browserProfiles"),
      icon: Globe2,
    },
    // Only deployments with the cloud offering get the Cloud page.
    ...(cloudEnabled
      ? [{ id: "cloud" as const, label: t("settings.cloud"), icon: Cloud }]
      : []),
  ...
  const [activeSection, setActiveSection] =
    useState<GlobalSettingsSection>("general");
  ...
  const [projectSaveState, setProjectSaveState] =
    useState<ProjectSettingsSaveState>(initialProjectSaveState);

  const activeLabel = isProjectSettings
    ? (projectSections.find((s) => s.id === activeProjectSection)?.label ??
      t("settings.project.identity"))
    : (globalSections.find((section) => section.id === activeSection)?.label ??
      t("settings.general"));
  ...
  useEffect(() => {
    if (settingsModal?.scope === "global")
      setActiveSection(settingsModal.section ?? "general");
        ...
        {displaySettings && (
          <div className="flex h-full min-h-0">
            <aside className="flex w-48 shrink-0 flex-col border-r border-(--color-border-settings-dialog-header) bg-card">
              ...
              <nav
                aria-label={t("settings.navSectionsAria")}
            ...
            <div className="flex min-w-0 flex-1 flex-col bg-card">
              <DialogHeader
                className={cn(
                  settingsDialogHeaderClass,
                  "flex h-auto shrink-0 flex-row items-center justify-between border-b-0",
                )}
              >
