...
vi.mock("../hooks/useAgentReadinessQuery", async (importOriginal) => {
  const actual =
    await importOriginal<typeof import("../hooks/useAgentReadinessQuery")>();
...
function requestBody() {
  const call = postMock.mock.calls.find(
    ([path]) => path === "/api/v1/orchestrators/delegate",
  );
...
beforeEach(() => {
  ...
  getMock.mockReset().mockImplementation(async (path: string) => {
    ...
    return {
      data: {
        status: "ok",
        project: { id: "proj-1", config: { worker: { agent: "claude-code" } } },
      },
      error: undefined,
    };
  });
  postMock.mockReset().mockImplementation(async (path: string) => {
    if (path === "/api/v1/agents/readiness/ensure")
      return { data: agentInventory, error: undefined };
    return {
      data: { ok: true, workerId: "worker-1", orchestratorId: "orch-1" },
      error: undefined,
    };
  });
});
...
describe("NewTaskDialog", () => {
  it("renders one continuous composer surface with a visible settings-style title", async () => {
    ...
    expect(screen.getByText("Create a new task")).toHaveClass(
      "settings-dialog-title",
    );
    expect(screen.queryByText("Runs with")).not.toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Close new task dialog" }),
    ...
    ).toHaveTextContent("Use Claude Code's default");
    expect(
      screen.getByRole("button", { name: "Add file" }),
    ).toBeInTheDocument();
    expect(
      screen.getByLabelText("Task").getAttribute("placeholder"),
    ).toBeTruthy();
  ...
  it("starts the original task naming the preselected project-default agent and optional model", async () => {
    ...
    await user.click(
      screen.getByRole("menuitem", {
        name: "Use “placeholder-model” as a custom model",
      }),
    );
  ...
  it("offers an explicit Terminal UI retry when Chat preflight fails", async () => {
      ...
      if (delegateAttempts === 1) {
        return {
          ...
          error: {
            ...
            message: "Claude Code needs login",
          },
        };
      }
    ...
    const retryBody = (
      delegateCalls()[1][1] as { body: Record<string, unknown> }
    ).body;
  ...
  it("shows an empty Model field for scratch projects and omits it from delegation", async () => {
    ...
    await user.type(
      ...
      "Build a quick prototype in scratch.",
    );
    await user.click(screen.getByRole("button", { name: "Start task" }));
