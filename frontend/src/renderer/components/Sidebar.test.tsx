...
beforeEach(() => {
  ...
  postMock.mockImplementation(
    async (route: string, { body }: { body?: { path?: string } }) => {
      if (route === "/api/v1/imports/validate") {
        return {
          data: projectValidation(body?.path ?? "/repo/new-project"),
          error: undefined,
        };
      }
...
describe("Sidebar", () => {
  ...
  it("defaults worker and orchestrator agents when creating a project", async () => {
    ...
    expect(
      screen.getByRole("dialog", { name: "Add a project" }),
    ).toBeInTheDocument();
    expect(window.ao!.app.chooseDirectory).not.toHaveBeenCalled();
    await user.click(
      screen.getByRole("button", { name: /^Import an existing project$/i }),
    );
  ...
  it("creates the selected local repository after backing out of a clone", async () => {
    ...
    await user.click(
      await screen.findByRole("button", {
        name: /^Import an existing project$/i,
      }),
    );
  ...
  it("prepares repositories with no commits before opening agent selection", async () => {
    ...
    postMock.mockResolvedValueOnce({
      data: {
        events: [
          { repoPath: "/repo/unborn", action: "git_commit", state: "success" },
        ],
  ...
  it("does not create the project when agent selection is cancelled after preparation", async () => {
    ...
    await screen.findByRole("dialog", { name: "Set up project" });
    await user.click(
      screen.getByRole("button", { name: "Close project agents dialog" }),
    );
    expect(onInitializeProject).not.toHaveBeenCalled();
    expect(onCreateProject).not.toHaveBeenCalled();
    expect(
  ...
  it("surfaces project preparation failures", async () => {
    ...
    await user.click(await screen.findByRole("button", { name: "Continue" }));
    expect(
      await screen.findByText(/failed while running Git initialization/i),
    ).toBeInTheDocument();
    expect(onCreateProject).not.toHaveBeenCalled();
    expect(onInitializeProject).not.toHaveBeenCalled();
  });
  ...
  it("can create a workspace project from the project add flow", async () => {
    ...
    await user.click(screen.getByLabelText("New project"));
    await user.click(
      screen.getByRole("button", { name: /^Import a workspace folder$/i }),
    );
    ...
    await user.click(screen.getByRole("button", { name: "Continue" }));
    expect(
      screen.getByRole("dialog", { name: "Set up workspace" }),
    ).toBeInTheDocument();
    await chooseOption(
      screen.getByRole("combobox", { name: "Worker agent" }),
      "Codex",
  ...
  it("starts the same inline rename from the session context menu", async () => {
    ...
    fireEvent.contextMenu(
      screen.getByRole("button", { name: "Open fix login" }),
    );
    const renameItem = await screen.findByRole("menuitem", {
      name: "Rename fix login",
    });
    ...
    expect(
      screen.getByRole("textbox", { name: "Rename fix login" }),
    ).toHaveFocus();
    expect(navigateMock).not.toHaveBeenCalled();
  });
  ...
  it.each(["", "fix login"])(
    "does not persist the no-op rename %j",
    async (nextName) => {
      const user = userEvent.setup();
      renderSidebar({ workspaces: [{ ...workspace, sessions: [session] }] });
  ...
  it("downloads the update when the available row is clicked", async () => {
    ...
    expect(
      screen.getByRole("button", { name: "Hide update v9.9.9 for 24 hours" }),
    ).not.toHaveClass("bg-interactive-hover");
  ...
  it("dismisses the current available update without downloading it", async () => {
    ...
    expect(
      screen.queryByLabelText("Download update v9.9.9"),
    ).not.toBeInTheDocument();
  ...
  it("pauses nested session drag contexts during a project drag", async () => {
      ...
      workspaces: [
        {
          ...
          id: "alpha",
          name: "Alpha",
          sessions: [{ ...session, id: "alpha-session", workspaceId: "alpha" }],
        },
        {
          ...workspace,
          id: "bravo",
          name: "Bravo",
          sessions: [{ ...session, id: "bravo-session", workspaceId: "bravo" }],
        },
      ],
    });

    expect(
      document.querySelectorAll('[data-dnd-context^="sidebar-sessions-"]'),
    ).toHaveLength(2);
    ...
    expect(
      screen.getAllByRole("button", { name: "Open fix login" }),
    ).toHaveLength(2);
  });
  ...
  it("commits a session drop within its project", () => {
    ...
    act(() =>
      dragEnds.get("sidebar-sessions-proj-1")?.({
        active: { id: "second" },
        over: { id: "first" },
      }),
    );
