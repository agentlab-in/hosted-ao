...
async function selectAcrossRows(
  ...
  const startRow = container.querySelector(
    `[data-row-index="${startIndex}"]`,
  ) as HTMLElement | null;
  const endRow = container.querySelector(
    `[data-row-index="${endIndex}"]`,
  ) as HTMLElement | null;
  if (!startRow || !endRow)
    throw new Error(
      `Expected diff rows at indices ${startIndex} and ${endIndex}`,
    );
  const startText = findTextNode(
    startRow.querySelector("span:last-child")!,
    false,
  );
  const endText = findTextNode(endRow.querySelector("span:last-child")!, true);
  if (!startText || !endText)
    throw new Error("Expected a text node inside the selected diff rows");
...
describe("SessionFilesView", () => {
  ...
  it("shows a loading search state instead of a false zero while the first file request is pending", () => {
    ...
    expect(
      screen.queryByPlaceholderText("Search 0 files"),
  ...
  it("loads the workspace files and requests detail for the selected file", async () => {
    ...
    expect(
      screen.queryByRole("button", { name: "Close files" }),
    ).not.toBeInTheDocument();
    expect(
    ...
    expect(
      screen.queryByRole("button", { name: "Download src/App.tsx" }),
    ).not.toBeInTheDocument();
    expect(
      screen.getByRole("button", { name: "Add feedback for file src/App.tsx" }),
    ).toBeInTheDocument();
    expect(
      screen.queryByRole("button", { name: "Diff layout" }),
    ).not.toBeInTheDocument();
  ...
  it("keeps a chat file focus request pending through a cold files load", async () => {
    ...
    expect(
      await screen.findByRole("button", { name: "Collapse docs/guide.md" }),
    ).toBeInTheDocument();
  ...
  it("reveals and focuses a file requested by a review comment", async () => {
    ...
    renderWithQuery(
      <SessionFilesView
        revealFile={{ path: "docs/guide.md", line: 7, requestId: 1 }}
        sessionId="sess-1"
      />,
    );
  ...
  it("scrolls and focuses the requested line in a multi-hunk review diff", async () => {
    ...
    renderWithQuery(
      <SessionFilesView
        revealFile={{ path: "src/App.tsx", line: 11, requestId: 1 }}
        sessionId="sess-1"
      />,
    );
  ...
  it("focuses the new-side row when revealing a context line in split view", async () => {
    ...
    await userEvent.click(
      screen.getByRole("button", { name: "Split diff view" }),
    );
  ...
  it("resolves a requested previous path to its renamed file", async () => {
    ...
    renderWithQuery(
      <SessionFilesView
        revealFile={{ path: "src/OldName.tsx", requestId: 1 }}
        sessionId="sess-1"
      />,
    );
    ...
    expect(
      await screen.findByRole("button", {
        name: "Collapse src/OldName.tsx -> src/NewName.tsx",
      }),
    ).toBeInTheDocument();
    await waitFor(() =>
      expect(getMock).toHaveBeenCalledWith(
  ...
  it("does not expand an unrelated file when a review path is missing", async () => {
    renderWithQuery(
      <SessionFilesView
        revealFile={{ path: "src/Missing.tsx", requestId: 1 }}
        sessionId="sess-1"
      />,
    );
    ...
    expect(
      screen.getByRole("button", { name: "Expand docs/guide.md" }),
    ).toBeInTheDocument();
    expect(
  ...
  it("filters and expands a changed file from the review list", async () => {
    ...
    await userEvent.type(
      await screen.findByPlaceholderText("Search 2 files"),
      "guide",
    );
    expect(
  ...
  it("keeps multiple files expanded at once", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "Expand docs/guide.md" }),
    );
    ...
    expect(
      screen.getByRole("button", { name: "Collapse src/App.tsx" }),
    ).toBeInTheDocument();
  });
  ...
  it("reports HEAD fallback when no base comparison is available", async () => {
    ...
    expect(
      await screen.findByText("No changes against HEAD."),
    ).toBeInTheDocument();
  ...
  it("uses the terminal foreground color for diff content", async () => {
    ...
    const codePane = (
      await screen.findByText(diffLine("const value = 1;"))
    ).closest(".session-files-diff-scrollbar");
  ...
  it("wraps long diff lines by default without a toggle", async () => {
    ...
    expect(await screen.findByText(diffLine("const value = 1;"))).toHaveClass(
      "whitespace-pre-wrap",
    );
    expect(
    ...
    expect(
      screen.queryByRole("button", { name: "Disable line wrapping" }),
    ).not.toBeInTheDocument();
  });
  ...
  it("switches between unified and side-by-side split diff", async () => {
    ...
    expect(
      screen.getByRole("button", {
        name: "Add feedback on old line 1 in src/App.tsx",
      }),
    ).toBeInTheDocument();
    expect(
      screen.getByRole("button", {
        name: "Add feedback on new line 1 in src/App.tsx",
      }),
    ).toBeInTheDocument();
    await userEvent.click(
      screen.getByRole("button", {
    ...
    expect(
      screen.getByRole("textbox", {
        name: "Feedback for src/App.tsx · old line 1",
      }),
    ).toBeInTheDocument();
  ...
  it("ignores split view for an added file — there is no old side to compare", async () => {
    ...
    const modifiedRow = screen
      .getByRole("button", { name: "Collapse src/App.tsx" })
      .closest("li");
    const addedRow = screen
      .getByRole("button", { name: "Collapse docs/guide.md" })
      .closest("li");
  ...
  it("sends inline line feedback to the session agent with precise diff context", async () => {
    ...
    const feedback = screen.getByRole("textbox", {
      name: "Feedback for src/App.tsx · new line 1",
    });
    ...
    await waitFor(() =>
      expect(postMock).toHaveBeenCalledWith(
        "/api/v1/sessions/{sessionId}/send",
        {
          params: { path: { sessionId: "sess-1" } },
          body: {
            message: expect.stringContaining("Reuse the shared value instead."),
          },
        },
      ),
    );
  ...
  it("opens whole-file feedback and cancels it with Escape", async () => {
    ...
    const feedback = screen.getByRole("textbox", {
      name: "Feedback for src/App.tsx · whole file",
    });
  ...
  it("renders changed files as one integrated review list instead of boxed cards", async () => {
    ...
    const activeRowButton = await screen.findByRole("button", {
      name: "Expand src/App.tsx",
    });
    ...
    expect(
      screen.getByLabelText("Session files").querySelector("header"),
    ).toHaveClass("h-10", "px-2");
  ...
  it("uses one vertical scroller for the file list and expanded diffs", async () => {
    ...
    const diffScroller = container.querySelector(
    ...
    expect(diffScroller?.parentElement).not.toHaveClass(
      "max-h-[min(620px,calc(100vh-18rem))]",
    );
  });
  ...
  it("routes vertical wheel gestures over a diff to the Files scroller immediately", async () => {
    ...
    const panelScroller = container.querySelector<HTMLElement>(
      "[data-files-scroll-root]",
    );
    const diffScroller = container.querySelector<HTMLElement>(
      ".session-files-diff-scrollbar",
    );
  ...
  it("shows binary-file feedback as a compact inline state", async () => {
    ...
    renderWithQuery(<SessionFilesView sessionId="sess-1" />);
    await userEvent.click(
      await screen.findByRole("button", { name: "Expand screenshot.png" }),
    );
  ...
  it("lets the caller toggle between rail and maximized layouts", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "Maximize files" }),
    );
  ...
  it("shows a minimize action while maximized", async () => {
    ...
    await userEvent.click(
      await screen.findByRole("button", { name: "Minimize files" }),
    );
  ...
  describe("diff selection -> send to agent", () => {
    ...
    it("opens the custom menu for a real drag selection across diff rows and suppresses the native menu", async () => {
      ...
      const scrollPane = container.querySelector(
        ".session-files-diff-scrollbar",
      ) as HTMLElement;
      ...
      const notCanceled = fireEvent.contextMenu(startRow, {
        clientX: 12,
        clientY: 34,
      });
      ...
      expect(
        screen.getByRole("menuitem", { name: "Explain" }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("menuitem", { name: "Make changes" }),
      ).toBeInTheDocument();
    });
    ...
    it("leaves the native context menu untouched when there is no active selection", async () => {
      ...
      const notCanceled = fireEvent.contextMenu(contentSpan, {
        clientX: 1,
        clientY: 1,
      });
    ...
    it("maps a single-side selection in split view to only that side's line", async () => {
      ...
      const scrollPane = oldSpan.closest(
        ".session-files-diff-scrollbar",
      ) as HTMLElement;
  ...
  describe("image diffs", () => {
    ...
    it("renders both sides of a changed image instead of the binary placeholder", async () => {
      ...
      renderWithQuery(<SessionFilesView sessionId="sess-1" />);
      await userEvent.click(
        await screen.findByRole("button", { name: "Expand docs/logo.png" }),
      );

      const before = await screen.findByAltText(
        "Before version of docs/logo.png",
      );
      const after = await screen.findByAltText(
        "After version of docs/logo.png",
      );
    ...
    it("shows only the after side for an added image", async () => {
      ...
      expect(
        await screen.findByAltText("After version of docs/logo.png"),
      ).toBeInTheDocument();
      expect(
        screen.queryByAltText("Before version of docs/logo.png"),
      ).not.toBeInTheDocument();
    });
    ...
    it("shows only the before side for a deleted image", async () => {
      ...
      expect(
        await screen.findByAltText("Before version of docs/logo.png"),
      ).toBeInTheDocument();
      expect(
        screen.queryByAltText("After version of docs/logo.png"),
      ).not.toBeInTheDocument();
    });
    ...
    it("falls back to a message when an image side fails to load", async () => {
      ...
      expect(
        await screen.findByText("Image preview could not be loaded."),
      ).toBeInTheDocument();
      expect(
        screen.getByAltText("After version of docs/logo.png"),
      ).toBeInTheDocument();
    });
  });
  ...
  describe("large diff virtualization", () => {
    ...
    it("opens a large diff without hanging, mounting far fewer DOM rows than the diff has", async () => {
      ...
      renderWithQuery(<SessionFilesView sessionId="sess-1" />);
      await userEvent.click(
        await screen.findByRole("button", { name: "Expand big.txt" }),
      );
  ...
  describe("git-state sections", () => {
      ...
      return {
        data: {
          ...
          commits: [
            {
              sha: "abc1234def",
              subject: "agent: add committed.go",
              author: "AO Agent",
    ...
    it("groups changed files into git-state sections with a commit list", async () => {
      getMock.mockImplementation(async (path: string) => {
        if (path === "/api/v1/sessions/{sessionId}/workspace/files")
          return sectionedWorkspaceFilesResponse();
        return { data: undefined };
      });
      ...
      expect(
        screen.getByRole("button", { name: /Staged/ }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /Unstaged/ }),
      ).toBeInTheDocument();
      expect(
        screen.getByRole("button", { name: /Untracked/ }),
      ).toBeInTheDocument();
    ...
    it("requests and renders each section's own diff for a partially staged file", async () => {
      getMock.mockImplementation(async (path: string, options?: unknown) => {
        ...
        if (path === "/api/v1/sessions/{sessionId}/workspace/file") {
          const query = (
            options as {
              params?: { query?: { path?: string; section?: string } };
            }
          ).params?.query;
          const diff =
      ...
      const [stagedRow] = await screen.findAllByRole("button", {
      ...
      await waitFor(() =>
        expect(getMock).toHaveBeenCalledWith(
          "/api/v1/sessions/{sessionId}/workspace/file",
          {
            params: {
              path: { sessionId: "sess-1" },
              query: { path: "README.md", section: "staged" },
            },
          },
        ),
      );
      await waitFor(() =>
        expect(getMock).toHaveBeenCalledWith(
          "/api/v1/sessions/{sessionId}/workspace/file",
          {
            params: {
              path: { sessionId: "sess-1" },
              query: { path: "README.md", section: "unstaged" },
            },
          },
        ),
      );
      expect(
        await screen.findByText(diffLine("only in staged")),
      ).toBeInTheDocument();
      expect(
        await screen.findByText(diffLine("only in unstaged")),
      ).toBeInTheDocument();
    });
