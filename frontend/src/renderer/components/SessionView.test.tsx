...
const codexAccountsQueryState = vi.hoisted(() => ({
...
vi.mock("../hooks/useCodexAccountsQuery", () => ({
  useCodexAccountsQuery: () => ({
    data: codexAccountsQueryState.data,
    isLoading: false,
  }),
}));
...
vi.mock("./ShellTopbar", () => ({
  ShellTopbar: ({
    ...
    <div
      ...
      data-testid="mock-session-topbar"
    >
      {sessionAction}
    </div>
  ),
  ...
  SessionChatSurface: ({
    ...
    >
      ...
      <button
        type="button"
        onClick={() =>
          onAuxiliaryTabOrderChange?.(["sh-a", "file:src/panel.tsx"])
        }
      >
      ...
      <button
        type="button"
        onClick={() =>
          onAuxiliaryTabOrderChange?.(["file:src/panel.tsx", "sh-a"])
        }
      >
      ...
      <div data-testid="shell-tabs">
        {shellTerminals.map((shell) => (
          <div key={shell.handleId}>
            ...
            <button
              onClick={() => onCloseShellTerminal?.(shell.handleId)}
              type="button"
            >
              close {shell.title}
            </button>
  ...
  SessionFileExplorer: ({
    ...
    <div>
      ...
      {!isMaximized && onOpenFile ? (
        <>
          <span>
            {revealRequest ? `rail preview ${revealRequest.path}` : "file tree"}
          </span>
          ...
          {revealRequest ? (
            <button
              type="button"
              onClick={() => onOpenFile(revealRequest.path)}
            >
  ...
  () => ({
    ...
    browserViewOptions: {
      current: undefined as
        | { active: boolean; sessionId: string; terminated: boolean }
        | undefined,
    },
...
function browserUnseen(sessionId: string): boolean {
  return Boolean(
    useUiStore.getState().inspectorSessions[sessionId]?.browserUnseen,
  );
}
...
describe("SessionView", () => {
  ...
  it("publishes which terminal the session pane is showing", () => {
    ...
    expect(useUiStore.getState().visibleTerminalKindBySession["sess-1"]).toBe(
      "worker",
    );
    ...
    view.unmount();
    expect(
      useUiStore.getState().visibleTerminalKindBySession["sess-1"],
    ).toBeUndefined();
  });
  ...
  it("activates a new terminal opened while a file tab is selected", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "pop out src/panel.tsx" }),
    );
    expect(
      await screen.findByTestId("session-file-workspace"),
    ).toHaveTextContent("src/panel.tsx");
  ...
  it.each([
    ...
    "keeps the git branch out of %s session's top bar",
    (_label, sessionId, mode, offersNewTerminal) => {
      ...
      expect(
        Boolean(screen.queryByRole("button", { name: "New terminal" })),
      ).toBe(offersNewTerminal);
    },
  );
  ...
  it("preserves mixed tab order across session navigation and interface changes", async () => {
    ...
    await screen.findByTestId("session-file-workspace");
    fireEvent.click(
      screen.getByRole("button", { name: "reorder auxiliary tabs" }),
    );
    ...
    view.rerender(<SessionView sessionId="sess-1" />);
    expect(
      screen.getByTestId("auxiliary-tab-order-tui-sess-1"),
    ).toHaveTextContent("sh-a|file:src/panel.tsx");
  });
  ...
  it("preserves a reordered reviewer position while the session is inactive", async () => {
    ...
    await screen.findByTestId("session-file-workspace");
    fireEvent.click(
      screen.getByRole("button", { name: "reorder reviewer tab" }),
    );
    expect(screen.getByTestId("auxiliary-tab-order-sess-1")).toHaveTextContent(
    ...
    view.rerender(<SessionView sessionId="sess-1" />);
    fireEvent.click(
      screen.getByRole("button", { name: "reorder visible tabs" }),
    );
  ...
  it("keeps a shell's reordered position when closing it fails", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "close session one shell" }),
    );
    view.rerender(<SessionView sessionId="sess-1" />);
  ...
  it.each([
    ...
    async (_label, sessionId, mode, targetMode, buttonName) => {
  ...
  it.each([
    ...
    async (_label, sessionId) => {
      ...
      expect(
        screen.queryByRole("menuitem", { name: "Switch to chat UI" }),
      ).not.toBeInTheDocument();
    },
  );
  ...
  it("shows the switch button when the adapter only reports a generic unsupported reason", async () => {
    ...
    expect(
      screen.getByRole("menuitem", { name: "Switch to chat UI" }),
    ).toBeInTheDocument();
  });
  ...
  it("keeps session chrome expanded when Browser is active", () => {
    useUiStore.setState({
      isSidebarOpen: true,
      inspectorSessions: {
        "sess-1": { initialized: true, isOpen: true, view: "browser" },
      },
    });
  ...
  it("renders no inspector panel or handle for orchestrator sessions", () => {
    ...
    expect(
      useUiStore.getState().inspectorSessions["sess-orch"],
    ).toBeUndefined();
  });
  ...
  it("previews docked files and only opens a center tab on explicit pop-out", () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "pop out src/App.tsx" }),
    );
  ...
  it("previews a review file target in the Files inspector without replacing the center", async () => {
    ...
    await waitFor(() => {
      expect(
        screen.getByText("rail preview src/panel.tsx"),
      ).toBeInTheDocument();
    });
    expect(
      screen.queryByRole("tab", { name: "panel.tsx" }),
    ).not.toBeInTheDocument();
    expect(
      screen.queryByTestId("session-file-workspace"),
    ).not.toBeInTheDocument();
  ...
  it("resolves a basename against workspace files before opening on a cold cache", async () => {
    ...
    fireEvent.click(
      screen.getByRole("button", { name: "view review basename" }),
    );

    await waitFor(() => {
      expect(
        screen.getByText("rail preview docs/notes.txt"),
      ).toBeInTheDocument();
    });
    expect(
      screen.queryByTestId("session-file-workspace"),
  ...
  it("keeps Summary selected for a terminated session with a stale previewUrl", () => {
    ...
    expect(inspectorButton()).toHaveAttribute("data-view", "summary");
    expect(
      useUiStore.getState().inspectorSessions["sess-1"]?.browserContentRevealed,
    ).toBeFalsy();
  });
