...
describe("SessionChatSurface link routing", () => {
  ...
  it.each([
    ...
    "does not acknowledge a switch presentation hidden by a %s",
    (_name, props, error) => {
      agentSwitchState.data = [
        {
      ...
      expect(visibilityMocks.presentation).toHaveBeenLastCalledWith(
  ...
  it.each([
    ...
    async (_name, overrides, outcome, _buttonDisabled) => {
      agentSwitchState.data = [
        {
          agentHandoffStatus: "not_attempted",
          fromHarness: "claude-code",
  ...
  it("ignores completed switch history when a stopped Chat controller reloads", () => {
    ...
    queryClient.setQueryData(agentSwitchesQueryKey(session.id), [
      historicalSwitch,
    ]);
